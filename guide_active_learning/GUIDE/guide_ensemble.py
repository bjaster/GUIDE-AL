...
class GUIDEEnsemble:
    ...
    def train_single_tree(
        self, train_df: pd.DataFrame, random_state: int
    ...
    def fit(self, train_df: pd.DataFrame, ensemble_size: int) -> None:
        self.train_df = train_df
        self.guide_ensemble = Parallel(n_jobs=1)(
            delayed(self.train_single_tree)(train_df, i) for i in range(ensemble_size)
        )
