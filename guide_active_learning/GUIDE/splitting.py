...
def spi_mixed_splitting_01(
    df: pd.DataFrame, cat_feature: int
    ...
    for cnt, A in enumerate(all_combs):
        ...
        w_g = (len(df1) / len(df)) * compute_gini_np(df1) + (
            len(df2) / len(df)
        ) * compute_gini_np(df2)
        combs[cnt] = A
        ginis[cnt] = w_g
...
def spi_mixed_splitting_02(
    df: pd.DataFrame, cat_feature: int, numerical_feature: int, csplits: List[Any]
) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for enu, A in enumerate(all_combs):
        ...
        for cnt, c in enumerate(csplits):
            ...
            w_g = (
                (len(df_1l) / len(df)) * compute_gini_np(df_1l)
                + (len(df_1r) / len(df)) * compute_gini_np(df_1r)
                + (len(df_2l) / len(df)) * compute_gini_np(df_2l)
                + (len(df_2r) / len(df)) * compute_gini_np(df_2r)
            )
...
def spi_categorical_splitting(
    df: pd.DataFrame, feature1: int, feature2: int
) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for enum, comb in enumerate(all_combs):
        ...
        for cnt, A in enumerate(combs_df1):
            ...
            w_g = (len(df_1l) / len(df)) * compute_gini_np(df_1l) + (
                len(df_1r) / len(df)
            ) * compute_gini_np(df_1r)
        ...
        for cnt, A in enumerate(combs_df2):
            ...
            w_g = (len(df_2l) / len(df)) * compute_gini_np(df_2l) + (
                len(df_2r) / len(df)
            ) * compute_gini_np(df_2r)
