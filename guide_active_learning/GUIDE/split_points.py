...
def select_numerical_feature(df: pd.DataFrame, feature: Union[List[str], str]) -> float:
    vals = np.array(df[feature].value_counts().index.sort_values())
    gini = np.ones(len(vals))
    for cnt, split in enumerate(vals):
        df1 = df[df[feature] <= split]
        df2 = df[df[feature] > split]
        wG = (len(df1) / len(df)) * compute_gini_pd(df1) + (
            len(df2) / len(df)
        ) * compute_gini_pd(df2)
...
def select_categorical_feature(
    df: pd.DataFrame, feature: Union[List[str], str]
) -> float:
...
def sp_main_feature(
    ...
    if df.dtypes[feature] in [float, int, "int64"]:
        min_gini = select_numerical_feature(df, feature)
    elif df.dtypes[feature] == object:
        min_gini = select_categorical_feature(df, feature)
    else:
...
def spi_numerical(
    df: pd.DataFrame, feature: List[str], target: str, d: int
) -> Tuple[str, float]:
    ...
    for cntc, c in enumerate(Sk1):
        ...
        for cnt, sp in enumerate(Sk2):
            ...
            wG1[cnt] = (len(df_ll) / len(df)) * compute_gini_np(df_ll) + (
                len(df_lr) / len(df)
            ) * compute_gini_np(df_lr)

        for cnt, e in enumerate(Sk2):
            df_rl = df_r[df_r[:, 1] <= e]
            df_rr = df_r[df_r[:, 1] < e]

            wG2[cnt] = (len(df_rl) / len(df)) * compute_gini_np(df_rl) + (
                len(df_rr) / len(df)
            ) * compute_gini_np(df_rr)
...
def spi_mixed(
    ...
    for cnt, c in enumerate(Sk1):
        ...
        if len(df_l) > 0 and len(df_r) > 0:
            feature_l, gini_l = spi_mixed_splitting_01(df_l, cat_feature)
            feature_r, gini_r = spi_mixed_splitting_01(df_r, cat_feature)
...
def spi_categorical(
    df: pd.DataFrame, feature: List[str], target: str
    ...
    sp1, delta1 = spi_categorical_splitting(df=df, feature1=0, feature2=1)

    if delta1 != 0:
        # do splitting the other way around only if no optimal solution is reached
        sp2, delta2 = spi_categorical_splitting(df=df, feature1=1, feature2=0)
...
def sp_interaction_features(
    ...
    d = 100
    try:
        if issubclass(df.dtypes[feature[0]].type, numbers.Number) and issubclass(df.dtypes[feature[1]].type, numbers.Number):
            split_feature, split_point = spi_numerical(df, feature, target, d)

        elif len(df.dtypes[feature].value_counts()) == 2:
            # one categorical and one numerical feature
            split_feature, split_point = spi_mixed(df, feature, target, d)

        elif df.dtypes[feature[0]] == object and df.dtypes[feature[1]] == object:
            # two categorical features
            split_feature, split_point = spi_categorical(df, feature, target)
