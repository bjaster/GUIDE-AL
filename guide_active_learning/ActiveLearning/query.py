...
def qbc_distance(
    *,
    ...
    distances = compute_distance(
        df_train=single_model.train_df, df_unlabeled=unlabeled_pool, max_distance=max_distance
    )
    ...
    *,
    ...
    if pool_synth:
        samples_ready = create_sobol_sequence(
            num_df=pd.concat([single_model.train_df, unlabeled_pool]).select_dtypes(include="number"),
            cat_df=pd.concat([single_model.train_df, unlabeled_pool]).drop(columns="target").select_dtypes(
                include="object"),
            random_state=random_state,
...
def qbc_exex_rf(
    *,
    ...
    if pool_synth:
        samples_ready = create_sobol_sequence(
            num_df=pd.concat([single_model.train_df, unlabeled_pool]).select_dtypes(include="number"),
            cat_df=pd.concat([single_model.train_df, unlabeled_pool]).drop(columns="target").select_dtypes(include="object"),
            random_state=random_state,
            n_samples=num_snowballs,
            active_learning_bounds=active_learning_bounds,
        )
    ...
    if type(unlabeled_pool) == (pd.DataFrame or pd.Series) and len(nodes) > 0 and volumes.size > 0 and uncertainties.size > 0:
        dp, result_qbc_exex = qbc_exec_loop(
            nodes=nodes,
            uncertainties=uncertainties,
            volumes=volumes,
            single_model=single_model,
            unlabeled_pool=unlabeled_pool,
            alpha_weight=np.array([alpha_weight, 1-alpha_weight, alpha_weight, 1-alpha_weight, -(1-alpha_weight)]),
            max_distance=max_distance,
            ensemble=ensemble,
        )
    else:
