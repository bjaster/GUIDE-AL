...
def compute_distance(
    ...
    if relevant_datapoints is not None:
        ...
        relevant_numerical = relevant_datapoints.select_dtypes(include="number").to_numpy()
        relevant_categorical = relevant_datapoints.select_dtypes(include="object").to_numpy()
    ...
    # compute numerical distance between unlabeled pool and training data (normalized by max_distance)
    numerical_distances = cdist(unlabeled_numerical, train_numerical, "minkowski", p=norm) / max_distance

    # compute categorical distances
    if unlabeled_categorical.size > 0:
        categorical_distances = np.ones((len(unlabeled_categorical), len(train_categorical)))
        for j in range(len(unlabeled_categorical)):
            intersection = np.sum(np.isin(train_categorical, unlabeled_categorical[j, :]), axis=1)
            union = np.array(
                [len(np.union1d(train_categorical[i, :], unlabeled_categorical[j, :])) for i in
                 range(len(train_categorical))]
            )
...
def compute_threshold_distance(
    ...
    if isinstance(threshold, float) and not isinstance(feature, list):  # univariate split
    ...
    if isinstance(node.feature, list):  # multivariate split
        whole_samples = compute_lda(
            df=pd.concat([df_train, df_unlabeled]), feature=cast(List[str], feature), target="target"
        )
        whole_samples = whole_samples.iloc[:, 0]
        min_val = np.min(whole_samples)
        max_val = np.max(whole_samples)
...
def create_sobol_sequence(
    ...
    if cat_array.size == 0:
        ...
        uniques = cast(
            List[np.ndarray],
            tuple([list(np.unique(cat_array[:, i])) for i in range(cat_array.shape[-1])]),
        )
        unique_categories = create_categorical_uniques_array(uniques)
        num_unique_categories = len(unique_categories)
        # determine the number sobol samples that must be drawn per unique feature combination
        ...
        for i, unique_category in enumerate(unique_categories):
            ...
            if i == 0:
                samples_ready = np.hstack(
                    (sobol_samples, np.tile(unique_categories[i, :], (len(sobol_samples), 1)))
                )
            else:
                tmp_cat = np.hstack(
                    (sobol_samples, np.tile(unique_categories[i, :], (len(sobol_samples), 1)))
    ...
    samples_ready = samples_ready.astype(dict(zip(num_df.columns, [float] * len(num_df.columns))))
...
def receive_tree_nodes(
    ...
    for node in nodes:
        if synth_pool.empty:
            if not node.pool.empty:
                nodes_new.append(node)
        else:
            path_check = [
                single_model.ison_prediction_path(
                    x=df.iloc[i, :], check_node=node
                )
    ...
    if isinstance(ensemble, GUIDEEnsemble):
        uncertainties = np.array(
            [
                ensemble.predict_uncertainties(node.pool, averaged=True)
                for node in nodes
            ]
        )
        ...
        uncertainties = np.array(
            [
                np.mean(calc_uncertainty(ensemble, node.pool))
                for node in nodes
            ]
        )
    else:
...
def qbc_exec_loop(
    ...
    # Calculate NN-distance between samples of unlabeled pool and already selected training samples
    distances = compute_distance(
        df_train=single_model.train_df, df_unlabeled=unlabeled_pool, relevant_datapoints=unlabeled_pool,
    ...
    # Run a loop over all nodes to assign real pool indexes and the corresponding uncertainty
    column_names = ['vol_per_sample', 'av_uncertainty', 'nn_dist', 'sample_uncertainty', 'threshold_dist']
    for k in range(len(nodes)):
        ...
        nodes[k].metrics = pd.DataFrame(initVal, index=range(len(nodes[k].unl_pool)), columns=column_names)
        nodes[k].metrics[column_names[0]] = volumes_std[k]
        nodes[k].metrics[column_names[1]] = uncertainties_std[k]
        nodes[k].metrics[column_names[2]] = min_dp_distances_std[path_check]
        nodes[k].metrics[column_names[3]] = uncertainty_query_std[path_check]
        ...
        if type(nodes[k].threshold) != str:
            ...
            if type(threshold_distances) != str:
                # weird case of two categorical values instead of one and therefore the first if
                # fails to detect categorical splits
                scaler = MinMaxScaler()
                # TODO threshold distance can have the same values for all queries if partition is small and split feature
                threshold_distances_std = scaler.fit_transform(threshold_distances.reshape(-1, 1)).flatten()
                nodes[k].metrics[column_names[4]] = threshold_distances_std
    ...
    scaler = MinMaxScaler()
    metrics_pool.iloc[:, 0:4] = scaler.fit_transform(metrics_pool.iloc[:, 0:4])
    weighted_metrics = np.multiply(metrics_pool.to_numpy(), alpha_weight)  # weight the metric-matrix by specific factors to controll exploration / exploitation

    if weighted_metrics[np.argmax(np.sum(weighted_metrics[:, 0:4], axis=1), axis=0), -1] == alpha_weight[-1]*initVal:  # query in a node of a categorical split is best
        dp = data_pool.reset_index().iloc[np.argmax(np.sum(weighted_metrics[:, 0:4], axis=1), axis=0), :]
    else:  # query in a node of a numerical or linear split is best
        dp = data_pool.reset_index().iloc[np.argmax(np.sum(weighted_metrics, axis=1), axis=0), :]
...
def calc_uncertainty(ensemble: RandomForestClassifier, df: pd.DataFrame) -> np.ndarray:
    ...
    if df.empty:
        ...
        # count the different predictions for each datapoint and compute the relative occurrences of them
        ...
        for i, predictions in enumerate(all_predictions):
            ...
            if len(counts) < ensemble.n_classes_:
                probs[i] = np.append(counts, np.zeros(ensemble.n_classes_ - len(counts))) / ensemble.n_estimators
            else:
