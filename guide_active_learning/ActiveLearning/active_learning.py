...
from guide_active_learning.ActiveLearning.query import (
    ...
    qbc_exex_rf, rf_uncertainty_query,
)
...
from guide_active_learning.ActiveLearning.active_learning_misc import one_hot_enc
...
def calculate_max_distance(df: pd.DataFrame) -> float:
    ...
    return np.max(scipy.spatial.distance.pdist(
        normed_al_df, metric='euclidean')
    )
...
def active_learning_step(
    *,
    ...
    max_distance: float,
    iteration: int = 0,
    alpha: float = 0.8,  # TODO: reduce alpha with every iterstion
